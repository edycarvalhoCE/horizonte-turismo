    ...
    <div className="min-h-screen bg-slate-100 flex font-sans text-slate-800">
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 p-4 space-y-2 mt-4">
          <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center px-4 py-3 rounded-lg ${activeTab === 'overview' ? 'bg-cyan-600 text-white' : 'hover:bg-slate-800'}`}><LayoutDashboard className="mr-3" /> Visão Geral</button>
          <button onClick={() => setActiveTab('packages')} className={`w-full flex items-center px-4 py-3 rounded-lg ${activeTab === 'packages' ? 'bg-cyan-600 text-white' : 'hover:bg-slate-800'}`}><Package className="mr-3" /> Pacotes</button>
          <button onClick={() => setActiveTab('bookings')} className={`w-full flex items-center px-4 py-3 rounded-lg ${activeTab === 'bookings' ? 'bg-cyan-600 text-white' : 'hover:bg-slate-800'}`}><CalendarDays className="mr-3" /> Reservas</button>
          <button onClick={() => setActiveTab('customers')} className={`w-full flex items-center px-4 py-3 rounded-lg ${activeTab === 'customers' ? 'bg-cyan-600 text-white' : 'hover:bg-slate-800'}`}><Users className="mr-3" /> Clientes</button>
        </nav>
      </aside>

      <main className="flex-1 ml-64 p-8">
        <header className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-slate-900 uppercase tracking-tight">{activeTab}</h1>
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              ...
              <h3 className="text-2xl font-bold">R$ 145.200</h3>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="bg-pink-500 p-6 rounded-xl shadow-sm text-white flex justify-between items-center">
              <div>
                <p className="text-sm opacity-80">Aniversariantes</p>
                <h3 className="text-2xl font-bold">{birthdaysToday.length} Hoje</h3>
              </div>
        ...
        {/* Tabelas de Clientes, Pacotes etc... (conforme implementado anteriormente) */}
        {activeTab === 'customers' && (
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase">Aniversário</th>
                </tr>
              </thead>
                ...
                {customers.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 flex items-center gap-2">
                      ...
                      {c.birthDate.slice(5, 10) === new Date().toISOString().slice(5, 10) && <Cake className="w-4 h-4 text-pink-500" />}
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-500">{c.email}</td>
                    <td className="px-6 py-4 text-sm">{new Date(c.birthDate + 'T00:00:00').toLocaleDateString('pt-BR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
